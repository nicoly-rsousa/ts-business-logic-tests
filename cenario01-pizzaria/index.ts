...
function calcularPedido(pedido: IPedido): IResultadoPedido {
    ...
    const totalPizzas = pedido.itens.reduce((acc, item) => acc + item.quantidade, 0)

    if (pedido.itens.length === 0 || totalPizzas === 0) {
    ...
    for (const item of pedido.itens) {
        ...
        subtotal += (pizza.preco + (item.bordaRecheada ? 8 : 0)) * item.quantidade
